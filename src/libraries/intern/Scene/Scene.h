...
class Scene
{
  public:
    ...
    void
    draw(const class Camera& camera, size_t viewportX, size_t viewportY, bool reflectionProbePass = false);
    void draw(
        const glm::mat4& view, const glm::mat4& proj, const glm::mat4& skyProj, size_t viewportX,
        size_t viewportY, bool reflectionProbePass = false);
    ...
    class Light* sun() const
    {
        return m_sunlight;
    }
    class ReflectionProbe* reflectionProbe() const
    {
        return m_reflectionProbe;
    }

    float skyExposure() const
    {
